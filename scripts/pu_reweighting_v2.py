...
PILEUP_DATA_DIR = os.path.join(configtools.getPath(),
                               'data/pileup')

class PUMCCalculator(object):
    """
    Read KAPPA skim outputs and write out their combined pile-up profile.

    Output: a ROOT file with a TH1D containing the pile-up profile.
    """

    def __init__(self, skim_output_dir):
        self._skim_output_dir = skim_output_dir
        if not os.path.isdir(self._skim_output_dir):
            raise OSError("Skim directory not found: {}".format(self._skim_output_dir))

    def produce_mc_pileup(self, target_file, read_max_n_skim_files=None):
        # prepend data dir path
        target_file = os.path.join(PILEUP_DATA_DIR, target_file)
        ...
        _files = os.listdir(self._skim_output_dir)
        ...
        for _file in _files:
            ...
            _tmp_chain.Add(os.path.join(self._skim_output_dir, _file))

        print "Merging PU distributions..."
        _tmp_chain.Draw("eventInfo.nPUMean >> pileup", "", "goff")
...
if __name__ == "__main__":
    ...
    if args.which == 'mc_pu_prof':
        _pu_mc_calc = PUMCCalculator(skim_output_dir=args.skim_dir)
