...
def ZJet():
    ...
    # get an existing one
        ...
        if any(getattr(options, attr, False) for attr in cli_conf_options):
            print "Resuming run, ignoring CLI options:", ", ".join(getattr(options, attr) for attr in cli_conf_options
    ...
    if options.root and not aborted:
        ...
        try:
            subprocess.call(["rot", "%s/%s.root" % (options.base, options.out)])
        except:
            pass
    return gctime
...
def populate_workdir(workdir_path, artus_json):
    ...
    transfer_dict = {
        "": {
            getEnv(): [
                # usermod will die after all batch scripts have been updated
                "cfg/gc/json_modifier.py", "cfg/gc/gc_base_usermod.conf",
