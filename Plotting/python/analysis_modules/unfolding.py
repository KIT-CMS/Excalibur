...
class Unfolding(analysisbase.AnalysisBase):
    ...
    ATTENTION: there is another analysis_module in Artus/HarryPlotter/python/analysis_modules/unfolding.py,
               but the one in Excalibur will be favorite!
    ...
    def do_unfolding(self, hResponse_input, hMeas, hTrue, hData, method, libRooUnfold_path, variation=0, write=False, unfold_file=None, iterations=4, regularization=1e-24):
        ...
        if write == True:
            # can be activated by using option 4 in RooUnfold: unfold.Hreco(4)
            
            #unfold.SetNToys(1000000)
            #unfold.RunToy()
            
            # get quantities provided by RooUnfold:
            recotruth_cov = unfold.Ereco()          # covariance matrix
            recotruth_hist = unfold.Hreco()         # reconstructed truth (i.e. unfolded) distribution
            recomeas_hist = unfold.HrecoMeasured()  # reconstructed measured distibution (i.e. response * unfolded)
            resp_matrix_m = response.Mresponse()    # normalized response matrix
            resp_matrix_e = response.Eresponse()    # normalized response matrix errors
            print recotruth_hist.GetBinContent(1)
            
            out_file = ROOT.TFile(unfold_file[0], 'RECREATE')
            
            # write existing quantities to file
            hData.Write("data_hist")                # (input) signal distribution
            hMeas.Write("reco_hist")                # (input) reco level distribution
            hTrue.Write("gen_hist")                 # (input) gen level distribution
            hResponse.Write("resp_hist")            # (input) response matrix
            recotruth_hist.Write("truth_hist")      # unfolded result
            recomeas_hist.Write("meas_hist")        # reconstructed measured result
            print hTrue.GetBinContent(1)
            
            # create histograms to write matrices:
            resp_matrix = hResponse.Clone("resp_matrix")
            cov_matrix = hResponse.Clone("cov_matrix")
            corr_matrix = hResponse.Clone("corr_matrix")
            
            for x in xrange(1, resp_matrix.GetNbinsX() + 1):
                for y in xrange(1, resp_matrix.GetNbinsY() + 1):
                    resp_matrix.SetBinContent(x, y, resp_matrix_m[x-1][y-1])
                    resp_matrix.SetBinError(x, y, resp_matrix_e[x-1][y-1])
                    cov_matrix.SetBinContent(x, y, recotruth_cov[x-1][y-1])
                    cov_matrix.SetBinError(x, y, 0.)
            # write correlation additionally to .txt file
            ...
            for x in xrange(1, corr_matrix.GetNbinsX() + 1):
                for y in xrange(1, corr_matrix.GetNbinsY() + 1):
                    ...
                    # the following line fills .txt file:
