...
PLOTSFOLDER = '/portal/ekpbms2/home/tberger/ZJtriple/ZJtriple_2019-11-06'
MCLIST = ['amc','hpp','mad',]
...
def invert_3Dhist(args=None, filename = ''):
    if filename == '':
...
def create_3Dhist(args=None, obs='zpt', cut='_jet1pt20', data='mad', match='',  postfix='', varquantity='', variation=0):
    if data == 'toy':
    ...
    h_reco.Write()
    h_gen.Write()
    g_reco = ROOT.TGraphAsymmErrors(h_reco)
    g_gen = ROOT.TGraphAsymmErrors(h_gen)
    g_reco.Write(obs+'_asym')
    g_gen.Write('gen'+obs+'_asym')
...
def unfold_by_tunfold_3Dhist(args=None,obs='zpt',cut='_jet1pt20',data='mad',mc='amc',match='', postfix='',varquantity='',variation=0):
    #if varquantity in [''] and not variation==0:
    #    print "WARNING: variations can not be computed for these variation quantities"
    #    variation = 0
    #if not 'toy' in mc and not varquantity == '_stats':
    #    variation = 0
    ...
    if not variation == 0:
      if not varquantity == '_stats':
        if varquantity in ['_Robs','_Ryz','_Ryj','_A','_F','_switch']:
          input_file_mc = PLOTSFOLDER+cut+postfix+'/variations/'+obs+'_'+mc+match+varquantity+VARIATIONSTRING[variation]+'.root'
        elif varquantity in ['_IDSF','_IsoSF','_TriggerSF','_JEC']:
          input_file_data = PLOTSFOLDER+cut+postfix+'/variations/'+obs+'_'+data+varquantity+VARIATIONSTRING[variation]+'.root'
        output_file = PLOTSFOLDER+cut+postfix+'/unfolded/'+obs+'_'+data+'_by_'+mc+match+varquantity+VARIATIONSTRING[variation]+'.root'
      else:
        output_file    = PLOTSFOLDER+cut+postfix+'/unfolded/'+obs+'_'+data+'_by_'+mc+match+varquantity+'.root'
    if '_PU' in varquantity:
...
def create_3Dunc(args=None, obs='zpt', cut='_jet1pt20', data='17Jul2018',match='', postfix='', varquantity='_total', N_toys=1000):
    var=varquantity
    ...
    if '_stats' in varquantity:
        ...
        for i in xrange(h_uncertainty.GetNbinsX()):
            ...
            for j in xrange(h_uncertainty.GetNbinsX()):
                ...
                h_cov.SetBinContent(i+1,j+1,h_sqr.GetBinContent(i+1,j+1)/N_toys-h_sum[i+1]/N_toys*h_sum[j+1]/N_toys)
        ...
        h_uncertainty.Write()
        h_uncertainty.Write('uncertainty'+varquantity+'Up')
        h_uncertainty.Write('uncertainty'+varquantity+'Down')
        h_cov.Write()
        #h_central.Write('unfolded'+obs+'_by_'+mc)
        g_central = ROOT.TGraphAsymmErrors(h_central)
        g_central.Write(obs+varquantity)
        ...
        for i in xrange(h_uncertainty.GetNbinsX()):
            ...
            h_central.SetBinError(i+1,max(abs(h_central[i+1]-h_up[i+1]),abs(h_central[i+1]-h_down[i+1])))
            h_uncertainty.SetBinContent(i+1,h_central.GetBinError(i+1)/h_central[i+1])
            h_uncertainty.SetBinError(i+1,0)
        f_out.cd()
        h_uncertainty.Write()
        ...
        for i in xrange(h_uncertainty.GetNbinsX()):
            ...
            h_central.SetBinError(i+1,h_uncertainty[i+1]*h_central[i+1])
        f_out.cd()
        h_uncertainty.Write()
        h_uncertainty.Write('uncertainty'+varquantity+'Up')
        h_uncertainty.Write('uncertainty'+varquantity+'Down')
        ...
        create_3Dhist(args,obs,cut,data,match,postfix,varquantity=var,variation=-1)
        unfold_by_tunfold_3Dhist(args,obs,cut,data,'toy',match,postfix)
        unfold_by_tunfold_3Dhist(args,obs,cut,data,'toy',match,postfix,varquantity,variation=1)
        unfold_by_tunfold_3Dhist(args,obs,cut,data,'toy',match,postfix,varquantity,variation=-1)
        input_file      = PLOTSFOLDER+cut+postfix+'/unfolded/'+obs+'_'+data+'_by_toy'+match+'.root'
        ...
        #input_file      = PLOTSFOLDER+cut+postfix+'/'+obs+'_'+data+'.root'
        #input_file_up   = PLOTSFOLDER+cut+postfix+'/variations/'+obs+'_'+data+var+'Up.root'
        #input_file_down = PLOTSFOLDER+cut+postfix+'/variations/'+obs+'_'+data+var+'Down.root'
        f_in      = ROOT.TFile(input_file,"READ")
        f_in_up   = ROOT.TFile(input_file_up,"READ")
        f_in_down = ROOT.TFile(input_file_down,"READ")
        h_central = f_in.Get('unfolded'+obs)
        ...
        h_uncertainty = h_central.Clone('uncertainty'+varquantity)
        h_uncertainty.Reset()
        h_uncertainty_up = h_uncertainty.Clone('uncertainty'+varquantity+'Up')
        h_uncertainty_down = h_uncertainty.Clone('uncertainty'+varquantity+'Down')
        g_central = ROOT.TGraphAsymmErrors(h_central)
        for i in xrange(h_uncertainty.GetNbinsX()):
            if h_up[i+1]>h_central[i+1]:
                if h_down[i+1]<h_central[i+1]:
                    h_uncertainty_up.SetBinContent(i+1,(h_up[i+1]-h_central[i+1])/h_central[i+1])
                    g_central.SetPointEYhigh(i,h_uncertainty_up[i+1]*h_central[i+1])
                    h_uncertainty_down.SetBinContent(i+1,(h_central[i+1]-h_down[i+1])/h_central[i+1])
                    g_central.SetPointEYlow(i,h_uncertainty_down[i+1]*h_central[i+1])
                else:
                    h_uncertainty_up.SetBinContent(i+1,(max(h_up[i+1],h_down[i+1])-h_central[i+1])/h_central[i+1])
                    g_central.SetPointEYhigh(i,h_uncertainty_up[i+1]*h_central[i+1])
                    h_uncertainty_down.SetBinContent(i+1,0)
                    g_central.SetPointEYlow(i, h_uncertainty_down[i+1]*h_central[i+1])
                
            elif h_up[i+1]<h_central[i+1]:
                if h_down[i+1]>h_central[i+1]:
                    h_uncertainty_up.SetBinContent(i+1,(h_down[i+1]-h_central[i+1])/h_central[i+1])
                    g_central.SetPointEYhigh(i,h_uncertainty_up[i+1]*h_central[i+1])
                    h_uncertainty_down.SetBinContent(i+1,(h_central[i+1]-h_up[i+1])/h_central[i+1])
                    g_central.SetPointEYlow(i,h_uncertainty_down[i+1]*h_central[i+1])
                else:
                    h_uncertainty_up.SetBinContent(i+1,0)
                    g_central.SetPointEYhigh(i,h_uncertainty_up[i+1]*h_central[i+1])
                    h_uncertainty_down.SetBinContent(i+1,(h_central[i+1]-min(h_up[i+1],h_down[i+1]))/h_central[i+1])
                    g_central.SetPointEYlow(i, h_uncertainty_down[i+1]*h_central[i+1])
                print "something went wrong in bin",i+1,h_up[i+1],h_central[i+1],h_down[i+1]
            #h_uncertainty.SetBinContent(i+1,max(max(abs(h_up[i+1]-h_central[i+1]),abs(h_down[i+1]-h_central[i+1]))/h_central[i+1],0))
            #h_uncertainty.SetBinContent(i+1,0.5*(g_central.GetErrorYhigh(i)+g_central.GetErrorYlow(i))/h_central[i+1])
            h_uncertainty.SetBinContent(i+1,0.5*(h_uncertainty_up[i+1]+h_uncertainty_down[i+1]))
            h_uncertainty.SetBinError(i+1,0)
        f_out.cd()
        h_uncertainty.Write()
        h_uncertainty_up.Write()
        h_uncertainty_down.Write()
        g_central.Write(obs+varquantity)
    elif varquantity in ['_JER']:
        unfold_by_tunfold_3Dhist(args,obs,cut,data,'toy',match,'')
        unfold_by_tunfold_3Dhist(args,obs,cut,data,'toy',match,'_JER')
        input_file      = PLOTSFOLDER+cut+'/unfolded/'+obs+'_'+data+'_by_toy'+match+'.root'
        input_file_var  = PLOTSFOLDER+cut+'_JER/unfolded/'+obs+'_'+data+'_by_toy'+match+'.root'
        f_in      = ROOT.TFile(input_file,"READ")
        f_in_var  = ROOT.TFile(input_file_var,"READ")
        h_central = f_in.Get('unfolded'+obs)
        h_var     = f_in_var.Get('unfolded'+obs)
        ...
        g_central = ROOT.TGraphAsymmErrors(h_central)
        for i in xrange(h_uncertainty.GetNbinsX()):
            g_central.SetPointEYhigh(i,abs(h_var[i+1]-h_central[i+1]))
            g_central.SetPointEYlow(i, abs(h_var[i+1]-h_central[i+1]))
            h_uncertainty.SetBinContent(i+1,g_central.GetErrorYhigh(i)/h_central[i+1])
            h_uncertainty.SetBinError(i+1,0)
        f_out.cd()
        h_uncertainty.Write()
        h_uncertainty.Write('uncertainty'+varquantity+'Up')
    ...
    #    h_central.Write(obs)
        ...
        unfold_by_tunfold_3Dhist(args,obs,cut,data,'toy',match,postfix)
        input_file = PLOTSFOLDER+cut+postfix+'/unfolded/'+obs+'_'+data+'_by_toy'+match+'.root'
        f_unf      = ROOT.TFile(input_file,"READ")
        h_central  = f_unf.Get('unfolded'+obs)
        for i in xrange(h_central.GetNbinsX()):
            h_central.SetBinError(i+1,h_uncertainty[i+1])
        g_central = ROOT.TGraphAsymmErrors(h_central)
        f_out.cd()
        g_central.Write(obs+varquantity)
        ...
        #unfold_by_tunfold_3Dhist(args,obs,cut,data,'toy',match,postfix)
        unfold_by_inversion_3Dhist(args,obs,cut,data,'toy',match,postfix)
        input_file = PLOTSFOLDER+cut+postfix+'/unfolded/'+obs+'_'+data+'_by_toy'+match+'.root'
        f_in       = ROOT.TFile(input_file,"READ")
        h_central = f_in.Get('unfolded'+obs)
        h_uncertainty = h_central.Clone('uncertainty'+varquantity)
    ...
    #    h_uncertainty.Write()
    #    h_central.Write(obs)
    elif varquantity in ['_systematic','_total']:
        input_file = PLOTSFOLDER+cut+postfix+'/unfolded/'+obs+'_'+data+'_by_toy'+match+'.root'
        f_in       = ROOT.TFile(input_file,"READ")
        h_central = f_in.Get('unfolded'+obs)
        for i in xrange(h_central.GetNbinsX()):
            h_central.SetBinError(i+1,0)
        g_central = ROOT.TGraphAsymmErrors(h_central)
        h_uncertainty = h_central.Clone('uncertainty'+varquantity)
        h_uncertainty.Reset()
        h_uncertainty_up   = h_uncertainty.Clone('uncertainty'+varquantity+'Up')
        ...
        varlist = ({'stat': ['_statistical'],
                    ...
                    'unf' : ['_stats_toy','_model_toy','_Robs','_Ryz','_Ryj','_A','_F','_switch'],
                    'jer' : ['_JER'],
                    'jec' : ['_JEC'],
                    #'muon': ['_MuScale'],
                    })
        for vkey in varlist:
          ...
          h_unc.Reset()
          h_unc_up   = h_unc.Clone("uncertainty_"+vkey+"Up")
          h_unc_down = h_unc.Clone("uncertainty_"+vkey+"Down")
          g_unc = ROOT.TGraphAsymmErrors(h_central)
          ...
          for v in varlist[vkey]:
            ...
            h_v_up         = f_in_unc.Get('uncertainty'+v+'Up')
            h_v_down       = f_in_unc.Get('uncertainty'+v+'Down')
            g_v            = f_in_unc.Get(obs+v)
            for i in xrange(h_uncertainty.GetNbinsX()):
                h_unc.SetBinContent(i+1,np.sqrt(h_unc[i+1]**2+h_v[i+1]**2))
                h_unc_up.SetBinContent(i+1,np.sqrt(h_unc_up[i+1]**2  +h_v_up[i+1]**2))
                h_unc_down.SetBinContent(i+1,-np.sqrt(h_unc_down[i+1]**2+h_v_down[i+1]**2))
                h_unc.SetBinError(i+1,0)
                h_unc_up.SetBinError(i+1,0)
                h_unc_down.SetBinError(i+1,0)
                g_unc.SetPointEYhigh(i,np.sqrt(g_unc.GetErrorYhigh(i)**2+g_v.GetErrorYhigh(i)**2))
                g_unc.SetPointEYlow(i, np.sqrt(g_unc.GetErrorYlow(i)**2+ g_v.GetErrorYlow(i)**2))
          if (not vkey == 'stat') or ('total' in varquantity):
            for i in xrange(h_uncertainty.GetNbinsX()):
                h_uncertainty.SetBinContent(i+1,np.sqrt(h_uncertainty[i+1]**2+h_unc[i+1]**2))
                h_uncertainty_up.SetBinContent(i+1,np.sqrt(h_uncertainty_up[i+1]**2+h_unc_up[i+1]**2))
                h_uncertainty_down.SetBinContent(i+1,-np.sqrt(h_uncertainty_down[i+1]**2+h_unc_down[i+1]**2))
                h_uncertainty.SetBinError(i+1,0)
                h_uncertainty_up.SetBinError(i+1,0)
                h_uncertainty_down.SetBinError(i+1,0)
                g_central.SetPointEYhigh(i,np.sqrt(g_central.GetErrorYhigh(i)**2+g_unc.GetErrorYhigh(i)**2))
                g_central.SetPointEYlow(i, np.sqrt(g_central.GetErrorYlow(i)**2+ g_unc.GetErrorYlow(i)**2))
          f_out.cd()
          h_unc.Write()
          h_unc_up.Write()
          h_unc_down.Write()
          g_unc.Write(obs+"_"+vkey)
        h_uncertainty.Write()
        h_uncertainty_up.Write()
        h_uncertainty_down.Write()
        g_central.Write(obs+varquantity)
...
def calculate_closure_chi2(args=None, obs='zpt', cut='_jet1pt20', data='mad', mc='mad',match='', postfix='', ybindex=None, ysindex=None):
...
def plot_closure_chi2(args=None, cut='_jet1pt20', data='17Jul2018', mc='hpp',match='', postfix=''):
  plots = []
  for obs in ['zpt','phistareta']:
...
def plot_uncertainty_3Dhist(args=None, cut='_jet1pt20', data='17Jul2018', mc='toy', match='', postfix=''):
  ...
  #varlist = [x+y for y in ['Up','Down'] for x in l_var ]
  varlist = l_var
  for obs in ['zpt','phistareta']:
    [l_obshists, h_reco, h_gen, h_response, l_ybinedges, l_obsbinedges] = prepare_3Dhist(args,obs)
    if not data in ['BCDEFGH','17Jul2018']:
        ...
        return
    filelist = [PLOTSFOLDER+cut+postfix+'/uncertainty/'+obs+'_'+data+match+'_systematic.root']
    ...
    d0 = ({
        ...
        'labels': ['Statistical','Total systematic','Luminosity','Background','Efficiency','Unfolding','JEC','JER'],#'Muon Scale'
        'alphas': [0.5],
        'y_errors': False,
        'y_lims': [0.0,0.1],
        #'y_lims': [-0.1,0.1],
        ...
        'colors': ['grey','black','red','blue','green','purple','orange','turquoise'],#,'p'],
        'x_label': (r'$\\mathit{p}_T^Z$' if obs =='zpt' else r'$\\mathit{\\phi}^{*}_{\\eta}$')+' Bin',
        'texts_size': [20],
        'labelsize': 25,
    })
...
def plot_crosssections_ratio(args=None,cut='_jet1pt20',data='17Jul2018',mc='NNLO',match='',postfix='',pdf='NNPDF31_nnlo_as_0118'):
  plots=[]
  for obs in ['zpt','phistareta']:
    [l_obshists, h_reco, h_gen, h_response, l_ybinedges, l_obsbinedges] = prepare_3Dhist(args,obs)
    unfold_by_tunfold_3Dhist(args,obs,cut,data,'toy',match,postfix)
    ...
    #write_data(args,obs,cut,data,match,postfix)
    #data_source = PLOTSFOLDER+cut+postfix+'/unfolded/'+obs+'_'+data+'_by_toy'+match+'.root'
    data_source  = PLOTSFOLDER+cut+postfix+'/uncertainty/'+obs+'_'+data+match+'_total.root'
    if mc in ['LO','NLO','NNLO']:
        write_theory(args,obs,cut,postfix,mc,pdf)
        mc_source   = PLOTSFOLDER+cut+postfix+'/theory/'+obs+'_'+mc+'_'+pdf+'.root'
    else:
        mc_source   = PLOTSFOLDER+cut+postfix+'/'+obs+'_'+mc+match+'.root'
    unc_source  = PLOTSFOLDER+cut+postfix+'/uncertainty/'+obs+'_'+data+match+'_systematic.root'
    ...
    d = ({
        ...
        #'zorder': len(namelist)*['stacked1']+len(namelist)*['stacked2']+len(namelist)*['stacked3']+len(namelist)*['stacked4'],
...
def plot_crosssections_3Dhist(args=None,cut='_jet1pt20',data='17Jul2018',match='',postfix='',pdf='',mclist=['mad','amc']):
  ...
  for obs in ['zpt','phistareta']:
    ...
    d0 = ({
        ...
        'files': filelist,
        'x_expressions': [obs+'_total']+len(mclist)*['gen'+obs+'_asym']+[obs+'_systematic'],
        'x_label': (r'$\\mathit{p}_T^Z$' if obs =='zpt' else r'$\\mathit{\\phi}^{*}_{\\eta}$')+' Bin',
        'x_errors': [1],
        'folders': [''],
        'lumis': [35.9],
...
def write_theory(args=None,obs='zpt',cut='_jet1pt20',postfix='',order='LO',pdf='NNPDF31_nnlo_as_0118',unc='',subprocess=False):
    ...
    if not subprocess:
      for ((xb,yb),(xs,ys)) in ylist:
        if pdf in ['NNPDF31_nnlo_as_0118','MMHT2014nnlo68cl','ABMP16als118_5_nnlo','CT14nnlo','NNPDF31_nlo_as_0118']:
            ...
            down_name_pdf   = "h"+dorder[order]+"100101"
            up_name_pdf     = "h"+dorder[order]+"100102"
            down_name_scale = "h"+dorder[order]+"100108"
            up_name_scale   = "h"+dorder[order]+"100109"
        else:
        ...
        for j in xrange(l_obshists[y_index].GetNbinsX()):
          obs_index = l_obsbinedges[y_index]+j+1
          h_gen.SetBinContent(obs_index,h_central[j+1]*35.9*1000*h_central.GetBinWidth(j+1))
          g_gen.SetPoint(obs_index-1,obs_index-0.5,h_gen[obs_index])
          #g_gen.SetPointEXhigh(obs_index-1,0.5)
          #g_gen.SetPointEXlow(obs_index-1,0.5)
        #g_gen = ROOT.TGraphAsymmErrors(h_gen)
        #for j in xrange(l_obshists[y_index].GetNbinsX()):
        #  obs_index = l_obsbinedges[y_index]+j+1
          if unc == 'pdf':
              h_gen.SetBinError(obs_index,h_gen[obs_index]*max(abs(h_up_pdf[j+1]),abs(h_down_pdf[j+1])))
              g_gen.SetPointEYhigh(obs_index-1,h_gen[obs_index]*h_up_pdf[j+1])
              g_gen.SetPointEYlow( obs_index-1,h_gen[obs_index]*h_down_pdf[j+1])
          elif unc == 'scale':
              h_gen.SetBinError(obs_index,h_gen[obs_index]*max(abs(h_up_scale[j+1]),abs(h_down_scale[j+1])))
              g_gen.SetPointEYhigh(obs_index-1,h_gen[obs_index]*h_up_scale[j+1])
              g_gen.SetPointEYlow( obs_index-1,h_gen[obs_index]*h_down_scale[j+1])
          else:
              h_gen.SetBinError(obs_index,h_gen[obs_index]*np.sqrt(max(abs(h_up_scale[j+1]),abs(h_down_scale[j+1]))**2+max(abs(h_up_pdf[j+1]),abs(h_down_pdf[j+1]))**2))
              g_gen.SetPointEYhigh(obs_index-1,h_gen[obs_index]*np.sqrt(h_up_scale[j+1]**2  +h_up_pdf[j+1]**2))
              g_gen.SetPointEYlow( obs_index-1,h_gen[obs_index]*np.sqrt(h_down_scale[j+1]**2+h_down_pdf[j+1]**2))
...
def plot_JEC(args=None, obs='zpt', cut='_jet1pt20', data='17Jul2018',postfix=''):
    plots=[]
    filelist = [PLOTSFOLDER+cut+postfix+'/'+obs+'_'+data+'.root']+[PLOTSFOLDER+cut+postfix+'/variations/'+obs+'_'+data+'_JEC'+x+'.root' for x in ['Up','Down']]
    filelist_binned = []
    for x in filelist:
        invert_3Dhists(args,x)
        filelist_binned.append(x.replace('.root','_binned.root'))
    for ybmin in [0.0,0.5,1.0,1.5,2.0]:
      for ysmin in [0.0,0.5,1.0,1.5,2.0]:
        ...
        d = ({
            ...
            'folders': [''],
            'x_expressions': [obs+namestring],
            'www': 'comparison_variations'+cut+'_'+data+'_JEC'+postfix+'/variations'+namestring,
            'y_log': True,
            'x_log': True,
            'x_label': obs,
            'x_errors': [1],
...
#def create_gendistributions_3Dhist(args=None, cut='_jet1pt20', mc='hpp',match='',postfix='_JER'):
    plots=[]
