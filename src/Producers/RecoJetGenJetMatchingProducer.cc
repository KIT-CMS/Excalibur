                                            ...
                                            ZJetSettings const& settings) const
{
	...
	{
		MatchCollection(event, product, settings, itlevel->first, settings.GetDeltaRMatchingRecoJetGenJet());
	}
}

void RecoJetGenJetMatchingProducer::MatchCollection(ZJetEvent const& event, ZJetProduct& product,
                                                    ...
                                                    double const deltaR) const
{
	...
	std::vector<int> matchResult = matchSort_Matrix<KLV, KJet>(*(event.m_genJets), event.m_genJets->size(), recoJets, recoJets.size(), deltaR);
