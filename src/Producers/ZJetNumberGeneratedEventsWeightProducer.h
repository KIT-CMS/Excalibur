...
public:
	std::string GetProducerId() const{
		return "ZJetNumberGeneratedEventsWeightProducer";
	}
	
	void Init(KappaSettings const& settings)
	{
		m_tolerance = 0.1;
	}
					...
					KappaSettings const& settings) const override
	{
		...
		{
			assert(specSettings.GetSampleReweightingCrossSections().size() == specSettings.GetSampleReweightingNEvents().size());

			//iterate over xsecs, find matching and get NEvents
			for (unsigned int i = 0; i < specSettings.GetSampleReweightingCrossSections().size(); i++)
			{
				if (std::abs(1. - (event.m_genLumiMetadata->xSectionInt / specSettings.GetSampleReweightingCrossSections().at(i))) < m_tolerance)
				{
					product.m_weights["numberGeneratedEventsWeight"] = (1.0 / specSettings.GetSampleReweightingNEvents().at(i));
					return;
				}
			}
			LOG(FATAL) << "No reweighting possible for internal cross-section " << event.m_genLumiMetadata->xSectionInt;
		}
		else
		{
