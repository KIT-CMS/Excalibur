...
void ZJetTreeConsumer::Init(ZJetSettings const& settings)
{
	...
	{
		return (product.GetValidJetCount(settings, event) > 0) ?  (product.jetpt_l1 / product.GetValidJet(settings, event, 0, "None")->p4.Pt()) : DefaultValues::UndefinedFloat;
	} );
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("jet1rc", [settings](ZJetEvent const& event, ZJetProduct const& product)
	{
		return (product.GetValidJetCount(settings, event) > 0) ?  (product.jetpt_rc / product.GetValidJet(settings, event, 0, "None")->p4.Pt()) : DefaultValues::UndefinedFloat;
	} );
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("jet1l2", [settings](ZJetEvent const& event, ZJetProduct const& product)
	{
		return (product.GetValidJetCount(settings, event) > 0) ?  (product.jetpt_l1l2l3 / product.jetpt_l1) : DefaultValues::UndefinedFloat;
	} );
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("jet1res", [settings](ZJetEvent const& event, ZJetProduct const& product)
	{
		return (product.GetValidJetCount(settings, event) > 0) ?  (product.jetpt_l1l2l3res / product.jetpt_l1l2l3) : DefaultValues::UndefinedFloat;
	} );
	...
	{
		return (product.GetValidJetCount(settings, event) > 0) ?  product.jetpt_l1 : DefaultValues::UndefinedFloat;
	} );
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("jet1ptl1l2l3", [settings](ZJetEvent const& event, ZJetProduct const& product)
	{
		return (product.GetValidJetCount(settings, event) > 0) ?  product.jetpt_l1l2l3res : DefaultValues::UndefinedFloat;
	} );
	...
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("mu1iso", [](event_type const& event, product_type const& product) {
		return product.m_validMuons.size() >= 1 ? product.m_validMuons[0]->pfIso() : DefaultValues::UndefinedFloat;
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("mu1sumchpt", [](event_type const& event, product_type const& product) {
		return product.m_validMuons.size() >= 1 ? product.m_validMuons[0]->sumChargedHadronPt : DefaultValues::UndefinedFloat;
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("mu1sumnhet", [](event_type const& event, product_type const& product) {
		return product.m_validMuons.size() >= 1 ? product.m_validMuons[0]->sumNeutralHadronEt : DefaultValues::UndefinedFloat;
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("mu1sumpet", [](event_type const& event, product_type const& product) {
		return product.m_validMuons.size() >= 1 ? product.m_validMuons[0]->sumPhotonEt : DefaultValues::UndefinedFloat;
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("mu1sumpupt", [](event_type const& event, product_type const& product) {
		return product.m_validMuons.size() >= 1 ? product.m_validMuons[0]->sumPUPt : DefaultValues::UndefinedFloat;
	});
	...
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e1looseid95", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsLooseVbtf95Electron(product.m_validElectrons[0], event, product);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e1mediumid95", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsMediumVbtf95Electron(product.m_validElectrons[0], event, product);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e1tightid95", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsTightVbtf95Electron(product.m_validElectrons[0], event, product);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e1tightid95", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsTightVbtf95Electron(product.m_validElectrons[0], event, product);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e1mvatrig", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsMVATrigElectron(product.m_validElectrons[0], event.m_electronMetadata);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e1mvanontrig", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsMVANonTrigElectron(product.m_validElectrons[0], event.m_electronMetadata);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e2looseid95", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsLooseVbtf95Electron(product.m_validElectrons[1], event, product);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e2mediumid95", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsMediumVbtf95Electron(product.m_validElectrons[1], event, product);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e2tightid95", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsTightVbtf95Electron(product.m_validElectrons[1], event, product);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e2tightid95", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsTightVbtf95Electron(product.m_validElectrons[1], event, product);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e2mvatrig", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsMVATrigElectron(product.m_validElectrons[1], event.m_electronMetadata);
	});
	LambdaNtupleConsumer<ZJetTypes>::AddFloatQuantity("e2mvanontrig", [](event_type const& event, product_type const& product) {
		return ValidElectronsProducer<ZJetTypes>::IsMVANonTrigElectron(product.m_validElectrons[1], event.m_electronMetadata);
	});
