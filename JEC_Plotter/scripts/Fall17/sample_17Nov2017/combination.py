    ...
    for _ch in args.channels: #('mm', 'ee'):
        for _runperiod in args.run_periods: #('B', 'C', 'D', 'E', 'F', 'BCDEF'):
            ...
            if _eta_binning_label is not None:
                _basename="zjet_combination_Fall17_JECV4_{}".format(_eta_binning_label)
            else:
                _basename="zjet_combination_Fall17_JECV4"

            _c = CombinationDirect(
                sample_data=SAMPLES['Data_Z{}_{}_Fall17_V4'.format(_ch, _runperiod)],
                sample_mc=SAMPLES['MC_Z{}_DYNJ_Fall17_V4'.format(_ch)],
