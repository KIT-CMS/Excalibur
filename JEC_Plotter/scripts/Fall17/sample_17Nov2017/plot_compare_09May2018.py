...
QUANTITIES['jet1chf'].bin_spec = BinSpec.make_equidistant(n_bins=51, range=(-0.2, 1.2))
QUANTITIES['jet1nhf'].bin_spec = BinSpec.make_equidistant(n_bins=51, range=(-0.2, 1.2))
QUANTITIES['jet1ef'].bin_spec = BinSpec.make_equidistant(n_bins=51, range=(-0.2, 1.2))
QUANTITIES['jet1mf'].bin_spec = BinSpec.make_equidistant(n_bins=51, range=(-0.2, 1.2))
QUANTITIES['jet1pf'].bin_spec = BinSpec.make_equidistant(n_bins=51, range=(-0.2, 1.2))
...
_SELECTION_CUTS = [
    #SELECTION_CUTS['finalcuts'],
    SELECTION_CUTS['basiccuts'] + _cut_final_no_eta + _pt_cut for _pt_cut in QUANTITIES['jet1pt'].make_cutsets_from_binspec(
            #BinSpec.make_from_bin_edges([30, 50, 200, 500, 1000])
            BinSpec.make_from_bin_edges([50, 200])
        )
]


if __name__ == "__main__":
    for _jecv_mc, _jecv_data in [("V10", "V10")]: #, ("V10", "V10_noMETmodification")]:
        for _channel in ("mm", "ee"):
            ...
            for _corr_level in ("L1L2L3",):
                ...
                _plot_collection = PlotProfiles(
                        ...
                        basename='profile_JEC{}'.format(_jecv_data,),
                        ...
                        cut_info_text_topleft_xy=(.03, .37),#(.35, .80)
                )
