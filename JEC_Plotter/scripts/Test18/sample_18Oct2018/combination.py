...
if __name__ == "__main__":
    ...
    p.add_argument("--channels", "-c", nargs="+", default=['mm'],
    ...
    for _ch in ('mm',): #args.channels: #('mm', 'ee'):
        for _runperiod in ('E',): #args.run_periods: #('B', 'C', 'D', 'E', 'F', 'BCDEF'):
            ...
            if _eta_binning_label is not None:
                _basename="zjet_combination_Test18_JEC{}_{}".format(_JECV_DATA, _eta_binning_label)
            else:
                _basename="zjet_combination_Test18_JEC{}".format(_JECV_DATA)

            _c = CombinationDirect(
                sample_data=SAMPLES['Data_Z{}_{}_Test18_JEC{}'.format(_ch, _runperiod, _JECV_DATA)],
